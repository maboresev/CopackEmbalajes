...
/* Trigger de pedido mínimo */
CREATE OR REPLACE TRIGGER pedido_minimo
  BEFORE INSERT ON LINEA_DE_PEDIDO
  FOR EACH ROW
BEGIN
    IF:NEW.CANTIDADPEDIDA<300
    THEN raise_application_error(-20600,'Se piden '||:NEW.CANTIDADPEDIDA||
         ' unidades. No se aceptan pedidos menores a 300 unidades');
    END IF;
END pedido_minimo;
...
/
  ...
  CREATE OR REPLACE FUNCTION "SYSTEM"."ASSERT_EQUALS" (salida BOOLEAN, salidaEsperada BOOLEAN) RETURN VARCHAR2 AS 
BEGIN
